...
3. The [Build VPS Toplevels](https://github.com/queze1/nixos-config/blob/main/.github/workflows/build-vps-toplevels.yml) GitHub action builds the NixOS configurations of VPSes, pushes them to a self-hosted a self-hosted [Attic](https://github.com/zhaofengli/attic) binary cache, and creates a GitHub release with their store paths. The [syste-puller](https://github.com/queze1/nixos-config/blob/main/modules/deployment/system-puller.nix) systemd script periodically polls the latest GitHub release. When a new NixOS configuration is published, it will fetch it from the binary cache and switch to it.
